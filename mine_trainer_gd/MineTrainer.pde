class MineTrainer{
  ...
  boolean win;
  MineTrainer(int boardW, int boardH, int numOfMines){
    this.cols = boardW;
    this.rows = boardH;
    ...
    this.target = new float[this.cols*this.rows];
    this.firstPicked = false;
    this.nn = new NeuralNetwork(7,boardW*boardH,boardW*boardH,1000,0.1);
    this.fitness = 0;
  ...
  MineTrainer(MineTrainer oldTrainer){
    ...
    this.w = oldTrainer.w;
    this.h = oldTrainer.h;
    this.numOfMines = oldTrainer.numOfMines;
    this.mines = new ArrayList<int[]>();
  ...
  float[] getTarget(){
    for(int i = 0; i < this.target.length;i++){
      if(checkMines(i/this.cols,i%this.cols)){
        this.target[i] = 0;
      } else{
        this.target[i] = 1;
      }
  ...
  boolean checkHighlights(int x, int y){
    for(int i = 0; i < this.highlights.size();i++){
      if(x == this.highlights.get(i)[0] && y == this.highlights.get(i)[1]){
        return(true);
      }
    }
    return(false);
  ...
  void pickSquare(int x, int y){
    if(this.board[x][y] < 0){
      if(this.firstPicked){
        ...
        while(this.mines.size() < this.numOfMines){
  ...
  void computerPick(){
    if(!this.gameComplete){
      ...
      for(int i = 0; i < inputs.length;i++){
        inputs[i] = this.board[(int)(i/this.cols)][i%this.cols];
      }
  ...
  void highlightSquare(int[] position){
    this.highlights.add(position);
  }
  void show(){
    for(int i = 0; i < this.board.length; i++){
      for(int j = 0; j < this.board[0].length;j++){
        if(checkHighlights(i,j)){
          fill(255,0,0);
        } else{
          fill(255,255,255);
